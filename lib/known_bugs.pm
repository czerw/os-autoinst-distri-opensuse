...
sub create_list_of_serial_failures {
 ...
 if (is_kernel_test()) {
        ...
        push @$serial_failures, {type => $type, message => 'WARNING CPU in kernel messages', pattern => qr/WARNING: CPU(?!.*kernel\/sched\/sched\.h)/, post_boot_type => 'hard', soft_on_expect_warn => 1};
        ...
        push @$serial_failures, {type => 'soft', message => 'Kernel Oops found', pattern => quotemeta '-[ cut here ]-', post_boot_type => 'hard', soft_on_expect_warn => 1};
